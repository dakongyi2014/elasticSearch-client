...
import com.alibaba.fastjson.{TypeReference, JSONObject, JSON}
...
object RestlasticSearchClient {
  ...
  object ReturnTypes {
    ...
    case class RawSearchResponse(hits: Hits) {
      ...
      def extractSource[T]: Seq[T] = {
        hits.hits.map(line=>{
        val toJsonObj= JSON.parse(line._source,JSON.DEFAULT_PARSER_FEATURE)
         toJsonObj.asInstanceOf[T]
        })
      }

      import java.util.{Map=>JMap}
      def sourceAsMap: Seq[Map[String, Any]] = hits.hits.map(line=>{
        val params:JMap[String,Object] = JSON.parseObject(line._source, new TypeReference[JMap[String, Object]](){})
        val scalaMap:Map[String,Any]=JavaConversions.mapAsScalaMap(params).toMap
        scalaMap
      })

      def highlightAsMaps: Seq[Map[String, Any]] = hits.hits.flatMap(_.highlight.map(line=>{
        val params:JMap[String,Object] = JSON.parseObject(line, new TypeReference[JMap[String, Object]](){})
        val scalaMap:Map[String,Any]=JavaConversions.mapAsScalaMap(params).toMap
        scalaMap
      }))
    }
    ...
    /*  def mappedTo[T: Manifest]: T = {
      ...
      def mappedTo[T]: T = {
        val jsonTree = JSON.parse(jsonStr)
        jsonTree.asInstanceOf[T]
      }
    }
    ...
    case class ElasticErrorResponse(error: String, status: Int) extends Exception(s"ElasticsearchError(status=$status): ${error.toString}")
